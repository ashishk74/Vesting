...
//    uint24 constant internal SECONDS_PER_MONTH = 2592000;
    ...
    for(uint i=0;i<_recipient.length;i++) {
      ...
      uint256 amountVestedPerDay = _amount[i].div(parameter[_name[i]].vestingDuration.mul(30));
    ...
    function calculateGrantClaim(uint256 _grantId) public view returns (uint16, uint256) {
        ...
        if (elapsedDays < tokenGrant.vestingCliff.mul(30)) {
            return (uint16(elapsedDays), 0);
        }

        // If over vesting duration, all tokens vested
        if (elapsedDays >= tokenGrant.vestingDuration.mul(30)) {
            uint256 remainingGrant = tokenGrant.amount.sub(tokenGrant.totalClaimed);
            return (tokenGrant.vestingDuration, remainingGrant);
        } else {
            uint16 timeVested = uint16(elapsedDays.sub(tokenGrant.daysClaimed));
            uint256 amountVestedPerDay = tokenGrant.amount.div(uint256(tokenGrant.vestingDuration.mul(30)));
            uint256 amountVested = uint256(timeVested.mul(amountVestedPerDay));
    ...
    function claimVestedTokens(uint256 _grantId) external {
        ...
        Grant storage tokenGrant = tokenGrants[_grantId];
        tokenGrant.daysClaimed = uint16(tokenGrant.daysClaimed.add(timeVested));
    ...
    function tokensVestedPerDay(uint256 _grantId) public view returns(uint256) {
        Grant storage tokenGrant = tokenGrants[_grantId];
        return tokenGrant.amount.div(uint256(tokenGrant.vestingDuration.mul(30)));
    }
